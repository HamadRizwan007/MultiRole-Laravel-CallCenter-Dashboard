<x-dashboard-layout>
    <x-slot name="pageTitle">Update Lead</x-slot>
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <form class="card" action="{{ route('leads.update', $lead->id) }}" enctype="multipart/form-data"
                method="POST">
                @csrf
                @method('PUT')
                <div class="card-header"><strong>Update Lead</strong></div>
                <div class="card-body">
                    @if ($errors->any())
                            ...
                            <select name="agent" id="selectCenterAgent">
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        @if (old('agent', $lead->agent_id) == $user->id) selected @endif>{{ $user->name }}
                    ...
                    <div class="form-group">
                        ...
                        <select name="quote" id="quote" class="form-control" required>
                            ...
                            @foreach (\App\Models\Lead::$quoteTypes as $key => $type)
                                <option value="{{ $key }}" @if (old('quote', $lead->quote) == $key) selected @endif>
                                    {{ ucwords($type) }}</option>
                            @endforeach
                        </select>
                        @error('quote')
                    ...
                    <div class="row">
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control @error('fname') is-invalid @enderror" id="fname"
                                ...
                                value="{{ old('fname', $lead->fname) }}" required>
                            @error('fname')
                                <div class="invalid-feedback font-weight-bold text-xsmall">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control @error('lname') is-invalid @enderror" id="lname"
                                ...
                                value="{{ old('lname', $lead->lname) }}" required>
                            @error('lname')
                                <div class="invalid-feedback font-weight-bold text-xsmall">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-8">
                            ...
                            <input class="form-control @error('dob') is-invalid @enderror" id="dob" name="dob"
                                ...
                                value="{{ \Carbon\Carbon::parse(old('dob', $lead->dob))->format('d-m-Y') }}"
                    ...
                    <div class="form-row">
                        ...
                        <div class="col-md-3">
                            <label for="checkNumber" class="d-block">Validate</label>
                    ...
                    <div class="row">
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control @error('address_line') is-invalid @enderror" id="address_line"
                                ...
                                value="{{ old('address_line', $lead->address_line) }}">
                            @error('address_line')
                                <div class="invalid-feedback font-weight-bold text-xsmall">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="form-row">
                            ...
                            <select name="state" id="state" class="@error('state') is-invalid @enderror">
                                ...
                                @foreach ($states as $state)
                                    <option value="{{ $state->name }}"
                                        @if (old('state', $lead->state) == $state->name) selected @endif>{{ $state->name }}</option>
                                @endforeach
                            </select>
                            @error('state')
                                <div class="invalid-feedback font-weight-bold text-xsmall">{{ $message }}</div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control @error('post_code') is-invalid @enderror" id="post_code"
                                ...
                                value="{{ old('post_code', $lead->zip_code) }}">
                            @error('post_code')
                                <div class="invalid-feedback font-weight-bold text-xsmall">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="form-group conditional-show" id="ownerTypeField">
                        ...
                        <select name="owner_type" id="owner_type"
                            ...
                            @foreach (\App\Models\Lead::$ownerTypes as $value)
                                <option value="{{ $value }}"
                                    @if (old('owner_type', $lead->owner_type) == $value) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                        @error('owner_type')
                            <div class="invalid-feedback font-weight-bold text-xsmall">{{ $message }}</div>
                    ...
                    <div class="form-group conditional-show" id="homeTypeField">
                        ...
                        <select name="home_type" id="home_type">
                            ...
                            @foreach (\App\Models\Lead::$homeTypes as $value)
                                <option value="{{ $value }}"
                                    @if (old('home_type', $lead->home_type) == $value) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                        @error('home_type')
                    ...
                    <div class="form-group conditional-show" id="insuredByField">
                        ...
                        <input class="form-control @error('insured_by') is-invalid @enderror" id="insured_by"
                            ...
                            value="{{ old('insured_by', $lead->insured_by) }}">
                        @error('insured_by')
                            <div class="invalid-feedback font-weight-bold text-xsmall">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group conditional-show" id="interestedInQuoteField" style="z-index: 2010;">
                        ...
                        <select name="quote_interested_in[]" id="quote_interested_in" multiple>
                            @foreach (\App\Models\Lead::$interestedInQuotes as $key => $value)
                                <option value="{{ $key }}"
                                    @if (in_array($key, old('quote_interested_in', $lead->quote_interested_in ?? []))) selected @endif>
                                    {{ $value }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group conditional-show" id="activeCheckingAccountField">
                        ...
                        <select name="have_active_checking_account" id="have_active_checking_account"
                            ...
                            @foreach (\App\Models\Lead::$activeCheckingAccountOptions as $key => $value)
                                <option value="{{ $key }}"
                                    @if (old('have_active_checking_account', $lead->have_active_checking_account) == $key) ) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group conditional-show" id="canPayThroughField" style="z-index: 1050;">
                        ...
                        <select name="can_pay_through[]" id="can_pay_through" multiple>
                            @foreach (\App\Models\Lead::$canPayThroughOptions as $key => $value)
                                <option value="{{ $key }}"
                                    @if (in_array($key, old('can_pay_through', $lead->can_pay_through ?? []))) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-6 conditional-show" id="hasActiveSsnField">
                            ...
                            <select name="has_active_ssn" id="has_active_ssn" class="form-control">
                                ...
                                @foreach (\App\Models\Lead::$yesNoOptions as $key => $value)
                                    <option value="{{ $key }}"
                                        @if (old('has_active_ssn', $lead->has_active_ssn) == $key) ) selected @endif>{{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-6 conditional-show" id="decisionMakerField">
                            <label for="decision_maker">Decision Make</label>
                            <select name="decision_maker" id="decision_maker" class="form-control">
                                <option value="">Select an Option</option>
                                @foreach (\App\Models\Lead::$yesNoOptions as $key => $value)
                                    <option value="{{ $key }}"
                                        @if (old('decision_maker', $lead->decision_maker) == $key) ) selected @endif>{{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-6 conditional-show" id="monthlyPayment_40_100Field">
                            ...
                            <select name="monthly_payment_40_100" id="monthly_payment_40_100" class="form-control">
                                ...
                                @foreach (\App\Models\Lead::$yesNoOptions as $key => $value)
                                    <option value="{{ $key }}"
                                        @if (old('monthly_payment_40_100', $lead->monthly_payment_40_100) == $key) ) selected @endif>{{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-6 conditional-show" id="anyCriticalIllnessField">
                            ...
                            <select name="any_critical_illness" id="any_critical_illness" class="form-control">
                                ...
                                @foreach (\App\Models\Lead::$yesNoOptions as $key => $value)
                                    <option value="{{ $key }}"
                                        @if (old('any_critical_illness', $lead->any_critical_illness) == $key) ) selected @endif>{{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-6 conditional-show" id="criticalIllnessField">
                            ...
                            <input type="text" name="critical_illness" id="critical_illness" class="form-control"
                                value="{{ old('critical_illness', $lead->critical_illness) }}"
                    ...
                    <div class="form-group conditional-show" id="carFieldA">
                        ...
                        <select name="cars_owned" id="cars_owned"
                            ...
                            @for ($i = 0; $i < 11; $i++)
                                <option value="{{ $i }}"
                                    @if (old('cars_owned', $lead->cars_owned) == $i) selected @endif>{{ $i }}</option>
                            @endfor
                        </select>
                        @error('cars_owned')
                            <div class="invalid-feedback font-weight-bold text-xsmall">{{ $message }}</div>
                    ...
                    <div class="form-group conditional-show" id="carFieldB">
                        ...
                        <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror"
                            placeholder="Enter any other extra information about the lead here...." rows="6">{{ old('description', $lead->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback font-weight-bold text-xsmall">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <select name="client" id="selectLeadClient">
                            ...
                            @foreach ($clients as $client)
                                <option value="{{ $client->id }}"
                                    @if (old('client', $lead->client_id) == $client->id) selected @endif>
                                    @if (auth()->user()->hasRole('admin'))
                    ...
                    <div class="form-group">
                        ...
                        <select name="assign_type" class="form-control @error('assign_type') is-invalid @enderror">
                            ...
                            @foreach (\App\Models\Lead::$assignTypes as $key => $type)
                                <option value="{{ $key }}"
                                    @if (old('assign_type', $lead->assign_type) == $key) selected @endif>{{ ucwords($type) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
