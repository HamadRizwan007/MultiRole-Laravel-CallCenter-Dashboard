<x-dashboard-layout>
    <x-slot name="pageTitle">Create Client</x-slot>
    ...
    <div>
        <form class="card" action="{{ route('client.store') }}" enctype="multipart/form-data" method="POST">
            @csrf
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control @error('agency') is-invalid @enderror" id="agency" name="agency" type="text" placeholder="Enter Your Agency" value="{{ old('agency') }}">
                        @error('agency')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select class="@error('lead_type') is-invalid @enderror" id="leadTypes" name="lead_type[]" multiple>
                            @foreach (\App\Models\Lead::$quoteTypes as $key => $type)
                                <option value="{{ $key }}" @if(in_array($key, old('lead_type', []))) selected @endif>{{ ucwords($type) }}</option>
                            @endforeach
                        </select>
                        @error('lead_type')
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select class="@error('requested_lead_type') is-invalid @enderror" id="leadRequestTypes" name="requested_lead_type[]" multiple>
                            @foreach (\App\Models\User::$leadRequestType as $key => $type)
                                <option value="{{ $key }}" @if(in_array($key, old('requested_lead_type', []))) selected @endif>{{ $type }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="timezone" id="timezone" class="form-control @error('timezone') is-invalid @enderror" required>
                            ...
                            @foreach (App\Models\User::$timezones as $key => $value)
                                <option value="{{ $key }}" @if(old('timezone') == $key) selected @endif>{{ $value }} -- {{ now()->setTimezone($key)->format('d M Y h:i A') }}</option>
                            @endforeach
                        </select>
                        @error('timezone')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    @if(env('SHOW_API_MAPPINGS'))
                        <div class="form-group col-md-6">
                            ...
                            <select name="api_type" id="apiType" class="form-control">
                                ...
                                @foreach (App\Models\User::$apiTypes as $key => $type)
                                    <option value="{{ $key }}" @if(old('api_type') == $key) selected @endif>{{ $type }}</option>
                                @endforeach
                            </select>
                            @error('api_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control @error('api_return_id_column') is-invalid @enderror" id="api_return_id_column"
                                name="api_return_id_column" type="text" value="{{ old('api_return_id_column') }}"
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <textarea name="api_url" id="apiUrl" class="form-control @error('api_url') is-invalid @enderror"
                                rows="10"
                                placeholder="Enter the fully qualified API URL here and make the mapping of lead information using the setup below">{{ old('api_url') }}</textarea>
                            @error('api_url')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <table class="table table-striped table-responsive-md">
                                ...
                                <tbody id="query_map">
                                    @foreach(old('api_query_map', []) as $key => $value)
                                        <tr id="queryMap_{{ $key }}">
                                            <td>{{ $key }}</td>
                                            <td>{{ $value }}</td>
                        ...
                        <div class="col-md-6">
                            ...
                            <table class="table table-striped table-responsive-md">
                                ...
                                <tbody id="static_map">
                                    @foreach(old('api_static_data', []) as $key => $value)
                                        <tr id="map_{{ $key }}">
                                            <td>{{ $key }}</td>
                                            <td>{{ $value }}</td>
                        ...
                        <div class="col-md-6">
                            <div>
                                ...
                                <table class="table table-striped table-responsive-md">
                                    ...
                                    <tbody id="data_map">
                                        @foreach(old('api_data_map', []) as $key => $value)
                                            <tr id="dataMap_{{ $key }}">
                                                <td>{{ $key }}</td>
                                                <td>{{ $value }}</td>
