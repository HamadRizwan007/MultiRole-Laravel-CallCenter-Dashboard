<x-dashboard-layout>
    <x-slot name="pageTitle">Edit Agent</x-slot>
    ...
    <div>
        <form class="card" action="{{ route('agent.update', $agent) }}" enctype="multipart/form-data" method="POST">
            @csrf
            @method('PUT')
            <div class="card-header"><strong>Update Agent</strong></div>
            <div class="card-body">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="fname">First Name</label>
                        <input class="form-control @error('fname') is-invalid @enderror" id="fname" name="fname" type="text" placeholder="Enter first name" value="{{ old('fname', $agent->fname) }}" required>
                        @error('fname')
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control @error('lname') is-invalid @enderror" id="lname" name="lname" type="text" placeholder="Enter last name" value="{{ old('lname', $agent->lname) }}" required>
                        @error('lname')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control @error('email') is-invalid @enderror" id="email" name="email" type="email" placeholder="Enter the call center email" value="{{ old('email', $agent->email) }}" required>
                        @error('email')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" type="text" placeholder="Enter the phone" value="{{ old('phone', $agent->phone) }}">
                        @error('phone')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" id="activeStatus" name="active_status" required>
                                ...
                                @foreach (\App\Models\User::$statuses as $key => $value)
                                    <option value="{{ $key }}" @if(old('active_status', $agent->status) == $key) selected @endif>{{ ucwords($value) }}</option>
                                @endforeach
                            </select>
                            @error('active_status')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="timezone" id="timezone" class="form-control @error('timezone') is-invalid @enderror" required>
                                ...
                                @foreach (App\Models\User::$timezones as $key => $value)
                                    <option value="{{ $key }}" @if(old('timezone', $agent->timezone) == $key) selected @endif>{{ $value }}</option>
                                @endforeach
                            </select>
                            @error('timezone')
                                <div class="invalid-feedback">{{ $message }}</div>
