<x-dashboard-layout>
    <x-slot name="pageTitle">Create Manager</x-slot>
    ...
    <div>
        <form class="card" action="{{ route('client-manager.store') }}" enctype="multipart/form-data" method="POST">
            @csrf
            <div class="card-header"><strong>Create Manager</strong></div>
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select id="viewPermissions" name="view_permissions[]" class="@error('view_permissions') is-invalid @enderror" multiple>
                            @foreach (App\Models\User::$viewPermissions as $value)
                                <option value="{{ $value }}" @if(in_array($value, old('view_permissions', []))) selected @endif>{{ ucwords($value) }}</option>
                            @endforeach
                        </select>
