<x-dashboard-layout>
    <x-slot name="pageTitle">Update Client Manager</x-slot>

    <x-slot name="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
        <li class="breadcrumb-item"><a href="{{ route('client-manager.index') }}">Managers</a></li>
        <li class="breadcrumb-item active">Update Client Manager</li>
    </x-slot>

    <div>
        <form class="card" action="{{ route('client-manager.update', $client_manager) }}" enctype="multipart/form-data" method="POST">
            @csrf
            @method('PUT')
            <div class="card-header"><strong>Update Client Manager</strong></div>
            <div class="card-body">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="fname">First Name</label>
                        <input class="form-control @error('fname') is-invalid @enderror" id="fname" name="fname" type="text" placeholder="Enter first name" value="{{ old('fname', $client_manager->fname) }}" required>
                        @error('fname')
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control @error('lname') is-invalid @enderror" id="lname" name="lname" type="text" placeholder="Enter last name" value="{{ old('lname', $client_manager->lname) }}" required>
                        @error('lname')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control @error('email') is-invalid @enderror" id="email" name="email" type="email" placeholder="Enter the call center email" value="{{ old('email', $client_manager->email) }}" required>
                        @error('email')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" type="text" placeholder="Enter the phone" value="{{ old('phone', $client_manager->phone) }}">
                        @error('phone')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 form-group text-left">
                        <label for="avatar">Manager Avatar</label>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="timezone" id="timezone" class="form-control @error('timezone') is-invalid @enderror" required>
                            ...
                            @foreach (App\Models\User::$timezones as $key => $value)
                                <option value="{{ $key }}" @if(old('timezone', $client_manager->timezone) == $key) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                        @error('timezone')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select id="viewPermissions" name="view_permissions[]" class="@error('view_permissions') is-invalid @enderror" multiple>
                            @foreach (App\Models\User::$viewPermissions as $value)
                                <option value="{{ $value }}" @if(in_array($value, old('view_permissions', $client_manager->view_permissions ?? []))) selected @endif>{{ ucwords($value) }}</option>
                            @endforeach
                        </select>
                        @error('view_permissions')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" id="activeStatus" name="active_status" required>
                                ...
                                @foreach (\App\Models\User::$statuses as $key => $value)
                                    <option value="{{ $key }}" @if(old('active_status', $client_manager->status) == $key) selected @endif>{{ ucwords($value) }}</option>
                                @endforeach
                            </select>
                            @error('active_status')
                                <div class="invalid-feedback">{{ $message }}</div>
    ...
    <x-slot name="scripts">
        <script>
            new SlimSelect({
                select: '#viewPermissions',
                placeholder: "Select the permissions that you manager can have..."
            });
        </script>
    </x-slot>
</x-dashboard-layout>
