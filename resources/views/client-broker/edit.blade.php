<x-dashboard-layout>
    <x-slot name="pageTitle">Edit Client Broker</x-slot>

    <x-slot name="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
        <li class="breadcrumb-item"><a href="{{ route('client-broker.index') }}">Client Brokers</a></li>
        <li class="breadcrumb-item active">Edit Client Broker</li>
    </x-slot>

    <div>
        <form class="card" action="{{ route('client-broker.update', $client_broker) }}" enctype="multipart/form-data"
            method="POST">
            @csrf
            @method('PUT')
            <div class="card-header"><strong>Edit Client Broker</strong></div>
            <div class="card-body">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="fname">First Name</label>
                        <input class="form-control @error('fname') is-invalid @enderror" id="fname" name="fname"
                            type="text" placeholder="Enter first name" value="{{ old('fname', $client_broker->fname) }}"
                            required>
                        @error('fname')
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control @error('lname') is-invalid @enderror" id="lname" name="lname"
                            type="text" placeholder="Enter last name" value="{{ old('lname', $client_broker->lname) }}"
                            required>
                        @error('lname')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control @error('email') is-invalid @enderror" id="email" name="email"
                            type="email" placeholder="Enter the email" value="{{ old('email', $client_broker->email) }}"
                            required>
                        @error('email')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone"
                            type="text" placeholder="Enter the phone" value="{{ old('phone', $client_broker->phone) }}">
                        @error('phone')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control @error('username') is-invalid @enderror" id="username" name="username" type="text" placeholder="Enter the username" value="{{ old('username', $client_broker->username) }}" required>
                        @error('username')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control @error('agency') is-invalid @enderror" id="agency" name="agency" type="text" placeholder="Enter Agency Name" value="{{ old('agency',$client_broker->agency) }}">
                        @error('agency')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="timezone" id="timezone" class="form-control @error('timezone') is-invalid @enderror" required>
                            ...
                            @foreach (App\Models\User::$timezones as $key => $value)
                                <option value="{{ $key }}" @if(old('timezone', $client_broker->timezone) == $key) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                        @error('timezone')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-6 form-group text-left">
                        ...
                        <div class="input-group">
                            ...
                            <div class="custom-file">
                                ...
                                <label class="custom-file-label" for="avatar">Select client broker avatar</label>
                            </div>
                        </div>
                    ...
                    <div class="form-group col-12">
                        ...
                        <textarea name="lead_instructions" id="leadInstructions" class="form-control @error('lead_instructions') is-invalid @enderror"
                            rows="10"
                            placeholder="Enter intructions for how the lead should be processed for the client!">{{ old('lead_instructions', $client_broker->lead_instructions) }}</textarea>
